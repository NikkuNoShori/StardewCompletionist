-- ============================================
-- Add catalog tracker to collection_progress.
-- Tracks the furniture/wallpaper catalogs the
-- player owns (Catalogue, Furniture, Retro, etc.).
-- ============================================

ALTER TABLE public.collection_progress
  ADD COLUMN IF NOT EXISTS catalog_checked JSONB NOT NULL DEFAULT '{}';

-- Update the unified getter to include catalog_checked.
...
-- Update the unified saver to persist catalog_checked.
...
BEGIN
  INSERT INTO collection_progress (
    ...
    monster_checked, catalog_checked, updated_at
  ) VALUES (
    auth.uid(),
