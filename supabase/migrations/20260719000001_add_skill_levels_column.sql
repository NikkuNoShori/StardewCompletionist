-- ============================================
-- Add skill-level tracking to collection_progress.
-- Stores the player's level (0-10) per skill, keyed
-- by skill name (farming, fishing, foraging, mining,
-- combat). Feeds the "Farmer Level" perfection line.
-- ============================================

ALTER TABLE public.collection_progress
  ADD COLUMN IF NOT EXISTS skill_levels JSONB NOT NULL DEFAULT '{}';

-- Update the unified getter to include skill_levels.
...
AS $$
  SELECT COALESCE(
    (SELECT jsonb_build_object(
      ...
      'skillLevels',         COALESCE(skill_levels, '{}'::jsonb)
    )
...
-- Update the unified saver to persist skill_levels.
...
BEGIN
  INSERT INTO collection_progress (
    ...
    monster_checked, catalog_checked, birthday_checked, skill_levels, updated_at
  ) VALUES (
    auth.uid(),
    ...
    COALESCE(p_data->'skillLevels',         '{}'::jsonb),
    now()
  )
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    skill_levels         = COALESCE(p_data->'skillLevels',         collection_progress.skill_levels),
    updated_at           = now();
END;
$$;
